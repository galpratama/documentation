---
title: "Setup layanan"
description: "Hubungkan KilatKoding ke Supabase, Resend, Midtrans, Doku, dan provider AI sesuai kebutuhan produk Anda."
mode: "wide"
---

## Prinsip dasar

Anda tidak harus menyalakan semua layanan. KilatKoding mendukung setup bertahap. Jika satu fitur belum siap, Anda bisa mematikannya lewat toggle env.

## 1. Supabase

Supabase adalah fondasi utama untuk auth dan database. Jika Anda ingin memakai login, dashboard, billing, admin, waitlist, avatar, atau audit log, mulai dari sini.
...
Untuk production, ganti domain lokal dengan domain live Anda.

### Kalau Anda ingin Google OAuth

1. Aktifkan provider Google di dashboard Supabase.
2. Masukkan Google Client ID dan Secret.
3. Tambahkan callback URL dari Supabase ke pengaturan OAuth app Anda di Google Cloud.

## 2. Resend

Gunakan Resend jika Anda ingin mengirim email dari form kontak atau email transaksional.
...
Gunakan Midtrans jika Anda ingin checkout lewat Snap.
...
4. Pastikan return URL user mengarah ke halaman order Anda. KilatKoding memakai `/order/[id]` dan punya fallback di `/payment/callback`.

## 4. Doku

Gunakan Doku jika Anda ingin checkout lewat JOKUL.
...
```text
https://domain-anda.com/api/webhooks/doku
```

## 5. AI provider

AI bersifat opsional. Kalau Anda tidak butuh AI, set `NEXT_PUBLIC_ENABLE_AI=false`.
...
<Tip>
Kalau Anda ingin launch cepat, aktifkan dulu Supabase dan satu payment provider. Resend dan AI bisa ditambahkan sesudah alur utama berjalan.
</Tip>
