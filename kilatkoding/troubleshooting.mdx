...
Dua pemeriksaan ini biasanya sudah cukup untuk menunjukkan apakah masalah Anda berasal dari env, feature toggle, atau konektivitas database.
...
<AccordionGroup>
  <Accordion title="Halaman auth atau dashboard tidak bekerja">
    ...
    Jika auth memang belum mau dipakai, set:
  ...
  <Accordion title="Akses admin tidak muncul">
    Pastikan user Anda ada di `user_roles` sebagai `admin`. `ADMIN_EMAILS` hanya membantu bootstrap awal.
  </Accordion>
...
Biasanya tabel yang perlu Anda lihat adalah `user_roles`, `subscriptions`, `payments`, `audit_logs`, dan `webhook_events`.
...
- Anda baru mengubah file `config/*`,
- Anda mengubah route handler atau komponen dashboard,
- Anda menambah integrasi baru di atas pola yang sudah ada.

<Note>
Kalau Anda sedang rebrand besar-besaran, masalah yang tampak seperti bug kadang hanya efek dari copy, navigasi, atau env toggle yang belum diselaraskan.
</Note>
